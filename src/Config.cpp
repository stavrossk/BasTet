...
namespace Bastet
{
  ...
  int HighScores::InsertHighScore
	  (int score, const std::string &scorer)
  {
  ...
  std::string Config::GetConfigFileName() const
  {
    ...
    return string(getenv("userprofile"))+RcFileName;
  ...
  std::string Config::GetHighScoresFileName() const
  {
		...
		string s = string( getenv("userprofile") ) + LocalHighScoresFileName;
		...
		if(result.empty())
		{
			...
			if(!ofs3.fail())
			{
				...
				cerr<<boost::str
					(boost::format
						("bastet: creating a new user-specific high scores file %1%\n")
						% s);
  ...
  Config::Config()
  {
		...
		po::options_description keyMappingOpts
			("Key mappings");
		...
		po::options_description highScoresOpts
			("High scores");
		...
		for(int difficulty=0;difficulty<num_difficulties;++difficulty)
			for(size_t i=0;i<HowManyHighScores;i++)
			{
				...
				highScoresOpts.add_options()
					...
					(str(scorer % difficulty % i).c_str(),po::value<string>()->default_value
						("No one played yet"),"Name of high scorer")
		...
		for(int difficulty=0;difficulty<num_difficulties;difficulty++)
		{
			...
			(
				...
				_hs[difficulty].begin(),_hs[difficulty].end()
