...
namespace Bastet{
	...
	void Ui::DropBlock(BlockType b, Well *w){
		...
		// Raphael R. - Replaced select()-call with WaitForSingleObject.
		HANDLE hStdIn = GetStdHandle(STD_INPUT_HANDLE);
		DWORD msecs = delay[_level] / 1000;

		while(1){ //break = tetromino locked
			DWORD rv = WaitForSingleObject(hStdIn, msecs);
			if(rv == WAIT_TIMEOUT){ //timeout
