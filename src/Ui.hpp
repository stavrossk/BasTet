...
namespace Bastet
{
	...
	class BorderedWindow
	{
		...
		public:
			BorderedWindow
				(int height, int width, int y=-1, int x=-1);
			///w and h are "inner" dimensions, excluding the border.
			///y and x are "outer", including the border. y=-1,x=-1 means "center"
  ...
  class Ui
  {
	...
	public:
		...
		int KeyDialog
			(const std::string &message); //asks for a single key
    
		
		int MenuDialog
			(const std::vector<std::string> &choices); //asks to choose one, returns index
